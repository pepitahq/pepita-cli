...
export async function run(args: string[]): Promise<void> {
  ...
  if (!slug) {
    ...
    for (const s of sites) {
      console.log(`  ${s.slug}`);
      console.log(`    draft: https://${s.slug}--draft.pepita.dev   live: https://${s.slug}.pepita.dev`);
    }
  ...
  console.log(`Draft:  https://${slug}--draft.pepita.dev`);
  console.log(`Live:   https://${slug}.pepita.dev`);
