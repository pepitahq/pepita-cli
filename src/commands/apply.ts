...
export async function run(args: string[]): Promise<void> {
  ...
  const confirm = async (plan: { writes: string[]; deletes: string[] }) => {
    ...
    const ans = (await rl.question('Apply as unsaved changes? [y/N] ')).trim().toLowerCase();
    rl.close();
    return ans === 'y' || ans === 'yes';
  };
  const r = await applyLocal(slug, dir, yes, confirm);
  console.log(
    `Applied to ${slug}: ${r.written} written, ${r.deleted} deleted (unsaved). Run \`pepita save ${slug}\` to save.`
  );
}
